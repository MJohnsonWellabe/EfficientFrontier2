...
- **Sales growth stays out of the baseline.** The frontier is a **multi-year** optimization (`MODEL_CANON.md §8`): only the per-product **2026 level** is sampled (`S.bounds`); growth is **target-seeking** — each product starts at `S.growthTarget[c]` (constant 2027–2030) and is cut only to stay feasible (`frontier.js → repairGrowth`: MS first toward `S.growthMin.MS`, negative allowed, then HI, then PN), so plans grow toward target and only go down when **C1 (min RBC 2026–2030 ≥ 4.0)** requires it. 2031–2035 held flat at 2030. Objective = **2026–2030 program PVDE** (`buildScen` `recNB` with `iyMax:2030`). The baseline (`computeBaseline`, anchors §1) must **never** read `S.growthTarget`. **C5–C8 are per-cohort** (each issue year 2026–2030 vs the original single-cohort thresholds −180/−150/yr4/yr10). The repair uses a `lite` `buildScen` (C1/C2/C3) for speed; `vnb.js` has an additive `iyMax` filter (opt-in; §1 byte-identical).
