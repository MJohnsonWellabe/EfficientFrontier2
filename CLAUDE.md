...
- **Sales growth stays out of the baseline.** The frontier is a **multi-year** optimization (`MODEL_CANON.md §8`), 15 LHS dims: per product the **2026 level** (`S.bounds`) plus a growth rate for **each year 2027–2030** drawn uniformly within **`[S.growthMin[c], S.growthTarget[c]]`** (`frontier.js → runSweep`, RNG order `levelA→grA→BANK`), so paths can rise-then-fall / front-load. Infeasible draws are **repaired** (`frontier.js → repairGrowth`: cut MS first, then HI, then PN; within a product **latest year first**, toward min — negative allowed for MS), chiefly against **C1 (min RBC 2026–2030 ≥ 4.0)**. 2031–2035 held flat at 2030. Objective = **2026–2030 program PVDE** (`buildScen` `recNB` with `iyMax:2030`). The baseline (`computeBaseline`, anchors §1) must **never** read `S.growthTarget`/`S.growthMin`. **C5–C8 are per-cohort** (each issue year 2026–2030 vs the original single-cohort thresholds −180/−150/yr4/yr10). The repair uses a `lite` `buildScen` (C1/C2/C3) for speed; `mkPath`/`repairGrowth` are exported so `runner/export-scalars.js` reproduces a scenario's path; `vnb.js` has an additive `iyMax` filter (opt-in; §1 byte-identical).
